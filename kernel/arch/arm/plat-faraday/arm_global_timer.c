...
void __init global_timer_register(void)
{
	struct clk *gt_clk;
	int err = 0;
	...
	gt_ppi = IRQ_GLOBALTIMER;
	if (!gt_ppi) {
		pr_warn("global-timer: unable to parse irq\n");
		return;
	}

	gt_base = (void __iomem *)PLAT_GTIMER_VA_BASE;
	if (!gt_base) {
		pr_warn("global-timer: invalid base address\n");
		return;
	}

	gt_clk = clk_get(NULL, "CPU");
	if (!IS_ERR(gt_clk)) {
	...
	gt_clk_rate = clk_get_rate(gt_clk)/2;
