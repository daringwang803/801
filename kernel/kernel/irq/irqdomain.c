			      ...
			      irq_hw_number_t hwirq)
{
	...
	if (hwirq < domain->revmap_size) {
#if 0   //zg-20201017		
		if(domain->fwnode == &of_find_node_by_path("/soc/gpio@54000000")->fwnode) {
			printk("$$$$$ %s %s %d hwirq:%d domain->revmap_size:%d, domain->linear_revmap[%d]:%d \n",
					__FILE__,
					__func__,
					__LINE__,
					hwirq,
					domain->revmap_size,hwirq,
					domain->linear_revmap[hwirq]);
			for(i=0;i<domain->revmap_size;i++) {
				printk("$$$$$  domain->linear_revmap[%d]:%d \n",
                                        i,
                                        domain->linear_revmap[i]);
