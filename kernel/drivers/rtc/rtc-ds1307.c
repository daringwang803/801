...
#else
		err = regmap_read(ds1307->regmap, RX8025_REG_CTRL1, &tmp);
		if (err) {
			dev_dbg(ds1307->dev, "read RX8025_REG_CTRL1 error %d\n", err);
			goto exit;
		}

		err = regmap_read(ds1307->regmap, RX8025_REG_CTRL2, &tmp);
		if (err) {
			dev_dbg(ds1307->dev, "read RX8025_REG_CTRL2 error %d\n", err);
			goto exit;
		}
#endif
